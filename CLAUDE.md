...
- `home/desktop.nix` — graphical environments: Hyprland, Quickshell, Ghostty, Alacritty, Mako, Waybar, wleave, uwsm, elephant
...
|---|---|---|
| `brian@liquidark` | x86_64-linux | common + arch + desktop |
| `brian@mac` | aarch64-darwin | common + darwin + dev-tools + desktop |
| `brian@pirateship` | aarch64-linux | common + headless |

### Config Files

`.config/` contains the raw config files for GUI apps (Hyprland, Quickshell/QML, Ghostty, Alacritty, etc.). These are linked into `~/.config/` via `home.file` in `home/desktop.nix`. Editing them in the repo takes effect after `home-manager switch`.

Neovim config is **not** managed here — it lives in a separate `~/code/kickstart.nvim` repo and is symlinked via `mkOutOfStoreSymlink` so edits are live without re-running home-manager.
